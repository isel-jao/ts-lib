...
function graph(entries: Record<string, string[]>): Map<string, Set<string>> {
  return new Map(Object.entries(entries).map(([node, deps]) => [node, new Set(deps)]));
}
...
describe("reverseDependencies", () => {
  ...
  it("gives every node an entry, empty when nothing depends on it", () => {
    expect(reverseDependencies(graph({ a: [], b: [] }))).toEqual(graph({ a: [], b: [] }));
  });

  it("inverts a single edge", () => {
    expect(reverseDependencies(graph({ a: [], b: ["a"] }))).toEqual(graph({ a: ["b"], b: [] }));
  });

  it("inverts a linear chain", () => {
    expect(reverseDependencies(graph({ a: [], b: ["a"], c: ["b"] }))).toEqual(
      graph({ a: ["b"], b: ["c"], c: [] })
    );
  });

  it("collects all dependents of a shared dependency (diamond)", () => {
    expect(reverseDependencies(graph({ a: [], b: ["a"], c: ["a"], d: ["b", "c"] }))).toEqual(
      graph({ a: ["b", "c"], b: ["d"], c: ["d"], d: [] })
    );
  });

  it("keeps a self-dependency", () => {
    expect(reverseDependencies(graph({ a: ["a"] }))).toEqual(graph({ a: ["a"] }));
  });
